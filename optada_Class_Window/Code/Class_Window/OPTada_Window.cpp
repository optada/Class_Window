...
void OPTada_Window::Update_WindowSizeWithBorders()
{
	...
	if (windowState != OPTadaE_WindowState_ForClassWindow::ENUM_WindowState_FullScreen) {
		...
		windowSize.height = window_rect.bottom - window_rect.top;  // window У
	}
	else {
		windowSize.width  = workplaceSize.width;  // window X
		windowSize.height = workplaceSize.height; // window У
	}

	ShowWindow(main_window_handle, SW_SHOW); // функция показывает окно (если оно не было показано ранее или обновляет его)
	...
	case WM_SYSKEYDOWN:
		...
		if ((((hookstruct->flags) >> 5) & 1)) {
			...
			case VK_TAB: { // ALT+TAB
				...
				//if (!Screen_Windowed)
				//{
				//	// при полноэкранном режиме и активации alt+tab окно сворачивается, при ативации - возвращается
